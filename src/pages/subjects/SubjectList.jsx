...
const defaultForm = {'name': '', 'short_name': '', 'subject_group': '', 'is_mapeh_component': '', 'is_active': ''}
...
export default function SubjectList() {
  ...
  const fetchData = async () => {
    setLoading(true)
    const { data: rows, error } = await supabase.from('subjects').select('*').order('name', { ascending: true })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing) {
        const { error } = await supabase.from('subjects').update(submitData).eq('id', editing.id)
  ...
  const columns = [
    ...
    { header: 'Subject Group', accessor: 'subject_group' },
    { header: 'MAPEH Component', accessor: row => row.is_mapeh_component ? 'Yes' : 'No' },
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Subject' : 'Add Subject'} size="lg">
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="text" name="subject_group" value={formData.subject_group || ''} onChange={handleChange} className="input-field" required />
            </div>
            <div className="flex items-center gap-2">
              <input type="checkbox" name="is_mapeh_component" checked={!!formData.is_mapeh_component} onChange={e => setFormData(p => ({...p, [is_mapeh_component]: e.target.checked}))} className="rounded" />
              <label className="text-sm font-medium text-gray-700">MAPEH Component</label>
            </div>
            <div className="flex items-center gap-2">
